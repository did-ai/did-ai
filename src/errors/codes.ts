export enum ErrorCode {
  DID_NOT_FOUND = "DID_NOT_FOUND",
  DID_DEACTIVATED = "DID_DEACTIVATED",
  DID_ALREADY_EXISTS = "DID_ALREADY_EXISTS",
  AUTH_REQUIRED = "AUTH_REQUIRED",
  INVALID_SIGNATURE = "INVALID_SIGNATURE",
  TIMESTAMP_EXPIRED = "TIMESTAMP_EXPIRED",
  NONCE_REPLAYED = "NONCE_REPLAYED",
  NETWORK_ID_MISMATCH = "NETWORK_ID_MISMATCH",
  SAID_MISMATCH = "SAID_MISMATCH",
  INVALID_DID_DOCUMENT = "INVALID_DID_DOCUMENT",
  NETWORK_NOT_FOUND = "NETWORK_NOT_FOUND",
  CROSS_NETWORK_REFERENCE = "CROSS_NETWORK_REFERENCE",
  IMMUTABLE_FIELD = "IMMUTABLE_FIELD",
  KEY_SEPARATION_VIOLATION = "KEY_SEPARATION_VIOLATION",
  AGENT_NOT_CALLABLE = "AGENT_NOT_CALLABLE",
  INVALID_VERSION_STATUS = "INVALID_VERSION_STATUS",
  INVALID_SKILL_BINDING = "INVALID_SKILL_BINDING",
  RATE_LIMITED = "RATE_LIMITED",
  NAMESPACE_INVALID = "NAMESPACE_INVALID",
  NAMESPACE_RESERVED = "NAMESPACE_RESERVED",
  INTERNAL_ERROR = "INTERNAL_ERROR",
  VALIDATION_FAILED = "VALIDATION_FAILED",
  FAMILY_NOT_FOUND = "FAMILY_NOT_FOUND",
  VERSION_NOT_FOUND = "VERSION_NOT_FOUND",
  MISSING_MIGRATION_GUIDE = "MISSING_MIGRATION_GUIDE",
  INVALID_VERSION_BUMP = "INVALID_VERSION_BUMP",
}

export const ERROR_STATUS: Partial<Record<ErrorCode, number>> = {
  [ErrorCode.DID_NOT_FOUND]: 404,
  [ErrorCode.DID_DEACTIVATED]: 410,
  [ErrorCode.DID_ALREADY_EXISTS]: 409,
  [ErrorCode.AUTH_REQUIRED]: 401,
  [ErrorCode.INVALID_SIGNATURE]: 401,
  [ErrorCode.TIMESTAMP_EXPIRED]: 401,
  [ErrorCode.NONCE_REPLAYED]: 401,
  [ErrorCode.NETWORK_ID_MISMATCH]: 422,
  [ErrorCode.SAID_MISMATCH]: 400,
  [ErrorCode.INVALID_DID_DOCUMENT]: 400,
  [ErrorCode.NETWORK_NOT_FOUND]: 404,
  [ErrorCode.CROSS_NETWORK_REFERENCE]: 422,
  [ErrorCode.IMMUTABLE_FIELD]: 422,
  [ErrorCode.RATE_LIMITED]: 429,
  [ErrorCode.INTERNAL_ERROR]: 500,
  [ErrorCode.VALIDATION_FAILED]: 400,
  [ErrorCode.FAMILY_NOT_FOUND]: 404,
  [ErrorCode.VERSION_NOT_FOUND]: 404,
  [ErrorCode.KEY_SEPARATION_VIOLATION]: 400,
  [ErrorCode.AGENT_NOT_CALLABLE]: 400,
  [ErrorCode.INVALID_VERSION_STATUS]: 400,
  [ErrorCode.INVALID_SKILL_BINDING]: 400,
  [ErrorCode.MISSING_MIGRATION_GUIDE]: 400,
  [ErrorCode.INVALID_VERSION_BUMP]: 400,
};
